...
vi.mock("@/lib/dash/api/driver-client", () => ({ postOrderProof, postOrderStatus: vi.fn() }));
...
describe("driver-store upload deduplication", () => {
  ...
  it("deduplicates concurrent saveProofAsync calls for the same proof type", async () => {
    ...
    const first = saveProofAsync("ORD-1", "signature", dataUrl);
    expect(isProofUploadInFlight("ORD-1", "signature")).toBe(true);

    const second = saveProofAsync("ORD-1", "signature", dataUrl);
    await Promise.all([first, second]);

    expect(isProofUploadInFlight("ORD-1", "signature")).toBe(false);
